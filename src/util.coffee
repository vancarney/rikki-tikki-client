...
$scope.getAPIUrl = ->
  _pcl = "#{$scope.getOptions 'PROTOCOL'}".toLowerCase()
  _host = $scope.getOptions 'HOST'
  _port = $scope.getOptions 'PORT'
  _path = "#{$scope.getOptions 'BASE_PATH'}".replace /^\//, ''
  _vers = $scope.getOptions 'API_VERSION'
  "#{_pcl}://#{_host}#{if _port != 80 then ':'+_port else ''}/#{_path}/#{_vers}"
